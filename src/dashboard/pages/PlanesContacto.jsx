      ...
      >
        <Paper elevation={6} sx={{ borderRadius: 4, padding: 4 }}>
          ...
          <Box mt={4} display="flex" flexDirection="column" gap={2}>
            <Button
              variant="contained"
              sx={{ backgroundColor: plan.color, fontWeight: 'bold' }}
              fullWidth
        ...
        <DialogContent dividers>
          {step === 1 ? (
            <Box display="flex" flexDirection="column" gap={2}>
              {["dni", "apellido", "nombre", "fechaNacimiento", "sexoBiologico", "email", "telefono"].map((field) => (
                <TextField
                  key={field}
                  label={field === "sexoBiologico" ? "Sexo (M/F)" : field.charAt(0).toUpperCase() + field.slice(1)}
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    const res = await axios.post(`https://sintacc-api-deploy.azurewebsites.net/api/Personas`, persona);
