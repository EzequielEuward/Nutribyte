...
export const FoodPage = () => {
  ...
  const gruposAlimenticios = useMemo(() => {
    const gruposSet = new Set(alimentos.map(a => a.grupoAlimenticio).filter(Boolean));
    return Array.from(gruposSet);
  }, [alimentos]);

  const filteredAlimentos = useMemo(() => {
    return alimentos.filter((food) => {
      const matchesNombre = food.nombre?.toLowerCase().includes(filters.nombre.toLowerCase());
      const matchesGrupo = filters.grupo === "" || food.grupoAlimenticio === filters.grupo;
      const matchesCalorias = food.calorias >= filters.calorias[0] && food.calorias <= filters.calorias[1];
      const matchesProteinas = food.proteinas >= filters.proteinas[0] && food.proteinas <= filters.proteinas[1];
  ...
  return (
    <DashboardLayout>
      ...
      <FoodFilters onFilterChange={handleFilterChange} gruposDisponibles={gruposAlimenticios} />
      ...
      {filteredAlimentos.length > 0 ? (
        view === "table" ? <FoodTable alimentos={filteredAlimentos} /> : <FoodCards alimentos={filteredAlimentos} />
      ) : (
